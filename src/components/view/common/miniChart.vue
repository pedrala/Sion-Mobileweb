...
export default {
  ...
  computed: {
    ...mapGetters([ 'isMaster', 'getSymbol', 'symbols' ])
  },
  data() {
    return {
      beforeSymbol: '',
  ...
  watch : {
    //'symbol' 은 null 값 감지하지 못함. null->'somedata'  somedata->'null'일 때는 symbol.id 로 변화 감지 가능 
    'symbol.id': {
      ...
      // console.log("oldVal:"+JSON.stringify(oldVal))
        ...
        if (self.beforeSymbol != '') {
          self.$socket.unregisterReal("TCS1", '.Feed', [ self.beforeSymbol ], "MiniChart");
        }
        self.getData();
      },
      deep: true,
  ...
  methods: {
    initAppChart: function () {
      ...
      console.log("miniChart", docWidth, docHeight, self.offset.left, self.offset.top, self.offset.width, self.offset.height);
      var beforeOptions = {
        ...
        if (queryData) {
          ...
          if (queryData.queryObj) {
            if (queryData.queryObj.OutBlock1 && queryData.queryObj.OutBlock1.length > 0) {
              ...
              self.$socket.registerReal("TCS1", '.Feed', [ self.symbol ], "MiniChart", self.callbackTCS1);
